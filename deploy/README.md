...
`SUPPLY_CHAIN_TRIVY_IMAGE`、`SUPPLY_CHAIN_COSIGN_IMAGE`、`SUPPLY_CHAIN_HELM_IMAGE` 使用 digest 固定。需要升级工具时,先用 Docker 拉取目标版本并确认 digest,再更新 `config/chaimir.env`。不要把可变 `latest` 作为仓库配置提交。
...
- `backend.yml` / `frontend.yml` / `images.yml`:路径触发 → lint+测试 → 构建 →
  Trivy 扫描(高危阻断)→ 推 Harbor并解析 digest → Cosign 签名和验签。三者共用 `ci/build-scan-sign-push` composite action,缺少 registry/robot/Cosign Secret 时在构建前显式失败。
