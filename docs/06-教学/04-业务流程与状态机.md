...
> 成绩导出必须使用真实 Excel 工作簿并分批读取 `course_grade`;导出分页批量由 `TEACHING_GRADE_EXPORT_BATCH_SIZE` 控制,启动时必须为正数,避免在模块代码中硬编码运行阈值。生成的 xlsx 只作为导出产物登记到基础横切层 `transfer_task`,不在 M6 另建导出表或把导出状态写入 `course_grade`。
