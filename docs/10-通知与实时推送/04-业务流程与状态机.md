# M10 通知与实时推送 — 业务流程与状态机

> Mermaid 描述通知发送、公告下发、实时推送、事件总线解耦。
> 最后更新:2026-05-29

---

## 1. 统一通知发送

```mermaid
flowchart TD
    A[模块触发事件] --> B{方式}
    B -->|直接调用| C[POST /send]
    B -->|事件总线| D[发 NATS 事件]
    D --> E[M10 订阅消费]
    C --> F[按 type 取模板渲染]
    E --> F
    F --> G{接收者偏好?}
    G -->|强制类/已开启| H[写 notification]
    G -->|已关闭| I[跳过]
    H --> J[Redis 未读+1]
    J --> K[WS 推红点]
```

> 未读红点以 `notification.is_read` 为权威状态;Redis 仅作实时缓存。缓存 miss 或键过期时,M10 必须按站内信表重建未读数后再返回。

---

## 2. 系统公告下发(不写放大)

```mermaid
flowchart LR
    A[管理员发布公告] --> B[写 system_announcement 一条]
    B --> C[按 scope 确定可见范围]
    C --> D[用户查公告列表]
    D --> E[对照 announcement_read 算未读]
    E --> F[阅读 → 写 announcement_read]
```

> 全校公告只存一条,不为每人复制,避免写放大。

---

## 3. 实时推送(WebSocket Hub)

```mermaid
sequenceDiagram
    participant C as 客户端
    participant Hub as M10 Hub
    participant Mod as 业务模块
    C->>Hub: WS 连接(JWT 鉴权)
    C->>Hub: subscribe(topics)
    Mod->>Hub: POST /push(topic, payload)
    Hub->>C: 推送给订阅该 topic 的在线连接
    Note over C,Hub: 心跳保活;断线重连;单端被踢则关闭
```

---

## 4. 事件总线解耦

```mermaid
flowchart LR
    A[模块产生事件] --> B[发 NATS 事件即返回]
    B --> C[M10 消费]
    C --> D{投递成功?}
    D -->|是| E[转站内信 / 实时推送]
    D -->|否| F[重试 N 次]
    F -->|仍失败| G[进死信队列 DLQ + 告警]
    Note over A,B: 发送方不阻塞,与投递解耦
```

> **投递可靠性(C3 修复)**:M10 消费事件失败按指数退避重试;重试耗尽进**死信队列(DLQ)**并触发运维告警人工介入。强制类通知(系统/成绩/审核)保证至少一次投递;站内信落库 + 实时推送双通道,实时漏推有站内信兜底。

---

## 5. 通知偏好与强制规则

```mermaid
flowchart TD
    A[准备发送] --> B{模板 force?}
    B -->|是 系统/成绩类| C[强制发送,忽略偏好]
    B -->|否| D{用户偏好开启?}
    D -->|是| C
    D -->|否| E[不发]
```
