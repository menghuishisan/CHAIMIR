...
> 事件总线入口消费 `notify.send.requested`、`notify.push.requested` 与业务完成事件(当前含 `experiment.scored`),载荷必须带真实 `tenant_id` 与 `trace_id`;失败由统一 eventbus 重试/DLQ 处理。业务事件携带稳定雪花事件 ID 时,M10 必须将其转为 `source_ref` 并依赖通知表唯一约束幂等落库。
