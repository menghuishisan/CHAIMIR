...
租户资源基线不由 M1 跨表写入。M1 在创建租户的同一事务写入 `tenant_provision_outbox`，提交后可靠发布
`identity.tenant.provisioned`；M2 订阅事件后创建自己表内唯一的 `tenant_quota`。M2 在配额读取和沙箱创建事务中
仍校验该不变量，只在记录缺失时写入环境变量声明的默认值，不覆盖管理员配置。全量租户轮询已删除。
...
sequenceDiagram
    ...
    C->>S: 登录(凭证)
    S->>S: 校验 + 吊销该账号其余会话(单端)
    S-->>C: Access(15min) + HttpOnly Refresh Cookie(7d)
    Note over C,S: 正常请求携带 Access
    C->>S: 请求(Access 已过期)
    S-->>C: 401 / code=11001
    C->>S: POST /auth/refresh({}+X-Chaimir-Refresh:1+Cookie)
    alt Refresh 有效
        S->>S: 轮转:签发新 Access+Cookie,旧 Refresh 失效
        S-->>C: 新 Access + 新 Cookie
    else Refresh 失效/被踢
