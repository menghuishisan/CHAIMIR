...
> 内置包在浏览器 Worker 内运行(它就是随平台版本交付的平台代码),扩展包在后端隔离容器内运行同一协议 —— 执行位置按代码来源分流,见 02 架构设计 §8。
> 最后更新:2026-08-06
...
阶段幂等入库(流程与三条硬约束见 06 文档 §4.1)。产物已提交入库,
...
- 对照推演:同一包起两个独立会话,同 seed 同参数,各自持有自己的 events,不共享状态(见 02 架构设计 §7)。
