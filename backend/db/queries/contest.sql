...
SELECT id, tenant_id, organizer_id, name, mode, match_mode, team_mode, signup_start, signup_end, start_at, end_at, freeze_minutes, rules, status, deleted_at, created_at, updated_at FROM contest WHERE id = @id AND deleted_at IS NULL;
...
UPDATE contest SET status = @status WHERE id = @id AND deleted_at IS NULL RETURNING id, tenant_id, organizer_id, name, mode, match_mode, team_mode, signup_start, signup_end, start_at, end_at, freeze_minutes, rules, status, deleted_at, created_at, updated_at;
...
SELECT id, tenant_id, contest_id, item_code, item_version, score, dynamic_score, battle_rule, seq FROM contest_problem WHERE contest_id = @contest_id ORDER BY seq ASC;

-- name: GetContestProblemByID :one
SELECT id, tenant_id, contest_id, item_code, item_version, score, dynamic_score, battle_rule, seq FROM contest_problem WHERE id = @id;
...
SELECT id, tenant_id, contest_id, name, invite_code, status, created_at FROM team WHERE id = @id;
...
SELECT id, tenant_id, team_id, account_id, member_tenant_id, is_leader FROM team_member WHERE team_id = @team_id ORDER BY is_leader DESC, id ASC;

-- name: GetTeamMember :one
SELECT id, tenant_id, team_id, account_id, member_tenant_id, is_leader FROM team_member WHERE team_id = @team_id AND account_id = @account_id;

-- name: LockTeam :one
UPDATE team SET status = @status WHERE id = @id RETURNING id, tenant_id, contest_id, name, invite_code, status, created_at;
...
SELECT id, tenant_id, contest_id, problem_id, team_id, submitter_id, content_ref, source_ref, judge_task_ref, passed, score, sandbox_ref, submitted_at FROM solve_submission WHERE id = @id;
...
UPDATE solve_submission SET passed = @passed, score = @score WHERE id = @id RETURNING id, tenant_id, contest_id, problem_id, team_id, submitter_id, content_ref, source_ref, judge_task_ref, passed, score, sandbox_ref, submitted_at;
...
SELECT id, tenant_id, contest_id, team_id, role, artifact_ref, version_no, is_active, submitted_at FROM battle_entry WHERE contest_id = @contest_id AND team_id = @team_id ORDER BY version_no DESC;

-- name: GetBattleEntryByID :one
SELECT id, tenant_id, contest_id, team_id, role, artifact_ref, version_no, is_active, submitted_at FROM battle_entry WHERE id = @id;
...
SELECT id, tenant_id, contest_id, entry_a_id, entry_b_id, sandbox_ref, result, score_delta, replay_ref, matched_at, finished_at FROM battle_match WHERE id = @id;
...
SELECT id, tenant_id, contest_id, team_id, score, solved_count, last_solve_at, rank, updated_at FROM ladder_rank WHERE contest_id = @contest_id AND team_id = @team_id;

-- name: ListLadderRanks :many
SELECT id, tenant_id, contest_id, team_id, score, solved_count, last_solve_at, rank, updated_at FROM ladder_rank WHERE contest_id = @contest_id ORDER BY rank ASC, score DESC LIMIT @limit_count OFFSET @offset_count;
...
SELECT id, tenant_id, contest_id, final_ranking, generated_at FROM contest_result_snapshot WHERE contest_id = @contest_id;
...
SELECT id, tenant_id, contest_id, team_id, type, evidence, action, operator_id, created_at FROM cheat_record WHERE contest_id = @contest_id ORDER BY created_at DESC LIMIT @limit_count OFFSET @offset_count;
...
SELECT id, tenant_id, type, name, config, default_level, enabled, last_sync_at, created_at, updated_at FROM vuln_source ORDER BY id DESC LIMIT @limit_count OFFSET @offset_count;

-- name: GetVulnSourceByID :one
SELECT id, tenant_id, type, name, config, default_level, enabled, last_sync_at, created_at, updated_at FROM vuln_source WHERE id = @id;

-- name: MarkVulnSourceSynced :one
UPDATE vuln_source SET last_sync_at = now() WHERE id = @id RETURNING id, tenant_id, type, name, config, default_level, enabled, last_sync_at, created_at, updated_at;
...
SELECT id, tenant_id, source_id, external_ref, title, level, runtime_mode, draft_body, prevalidate_status, prevalidate_detail, content_item_code, content_item_version, status, created_at, updated_at FROM vuln_problem WHERE id = @id;
