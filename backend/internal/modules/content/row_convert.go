...
func itemFromRow(row sqlcgen.ContentItem) Item {
	return Item{
		...
		Type:            row.Type,
		Title:           row.Title,
		CategoryID:      int64Value(row.CategoryID),
...
func categoryFromRow(row sqlcgen.ContentCategory) Category {
	return Category{ID: row.ID, TenantID: row.TenantID, ParentID: int64Value(row.ParentID), Name: row.Name, Sort: row.Sort, CreatedAt: timeFromPg(row.CreatedAt), UpdatedAt: timeFromPg(row.UpdatedAt)}
}
...
func paperFromRow(row sqlcgen.Paper) (Paper, error) {
	...
	return Paper{ID: row.ID, TenantID: row.TenantID, Name: row.Name, AuthorID: row.AuthorID, GenMode: row.GenMode, GenCriteria: criteria, CreatedAt: timeFromPg(row.CreatedAt), UpdatedAt: timeFromPg(row.UpdatedAt)}, nil
}

// paperItemFromRow 转换卷题关联行。
func paperItemFromRow(row sqlcgen.PaperItem) PaperItem {
	return PaperItem{ID: row.ID, TenantID: row.TenantID, PaperID: row.PaperID, ItemCode: row.ItemCode, ItemVersion: row.ItemVersion, Score: row.Score, Seq: row.Seq, CreatedAt: timeFromPg(row.CreatedAt)}
}
