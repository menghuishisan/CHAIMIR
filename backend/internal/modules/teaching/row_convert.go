...
func chapterFromRow(row sqlcgen.Chapter) Chapter {
	return Chapter{ID: row.ID, TenantID: row.TenantID, CourseID: row.CourseID, Title: row.Title, Sort: row.Sort, CreatedAt: timeFromPg(row.CreatedAt), UpdatedAt: timeFromPg(row.UpdatedAt)}
}
...
func lessonFromRow(row sqlcgen.Lesson) (Lesson, error) {
	...
	return Lesson{ID: row.ID, TenantID: row.TenantID, ChapterID: row.ChapterID, Title: row.Title, ContentType: row.ContentType, ContentRef: ref, Sort: row.Sort, CreatedAt: timeFromPg(row.CreatedAt), UpdatedAt: timeFromPg(row.UpdatedAt)}, nil
}

// memberFromRow 转换课程成员表行。
func memberFromRow(row sqlcgen.CourseMember) CourseMember {
	return CourseMember{ID: row.ID, TenantID: row.TenantID, CourseID: row.CourseID, StudentID: row.StudentID, JoinedAt: timeFromPg(row.JoinedAt), JoinMode: row.JoinMode}
}
...
func assignmentFromRow(row sqlcgen.Assignment) (Assignment, error) {
	...
	return Assignment{ID: row.ID, TenantID: row.TenantID, CourseID: row.CourseID, Title: row.Title, ChapterID: int64Value(row.ChapterID), DueAt: timeFromPg(row.DueAt), MaxAttempts: row.MaxAttempts, LatePolicy: row.LatePolicy, LatePenalty: penalty, Status: row.Status, CreatedAt: timeFromPg(row.CreatedAt), UpdatedAt: timeFromPg(row.UpdatedAt)}, nil
}

// assignmentItemFromRow 转换作业题目引用表行。
func assignmentItemFromRow(row sqlcgen.AssignmentItem) AssignmentItem {
	return AssignmentItem{ID: row.ID, TenantID: row.TenantID, AssignmentID: row.AssignmentID, ItemCode: row.ItemCode, ItemVersion: row.ItemVersion, Score: row.Score, Seq: row.Seq, GradingMode: row.GradingMode, JudgerCode: textValue(row.JudgerCode), CreatedAt: timeFromPg(row.CreatedAt)}
}
...
func submissionFromRow(row sqlcgen.Submission) (Submission, error) {
	...
	return Submission{ID: row.ID, TenantID: row.TenantID, AssignmentID: row.AssignmentID, StudentID: row.StudentID, AttemptNo: row.AttemptNo, ContentRef: ref, JudgeTaskRef: textValue(row.JudgeTaskRef), AutoScore: int32Value(row.AutoScore), ManualScore: int32Value(row.ManualScore), FinalScore: int32Value(row.FinalScore), Comment: textValue(row.Comment), IsLate: row.IsLate, Status: row.Status, SubmittedAt: timeFromPg(row.SubmittedAt)}, nil
}
...
func draftFromRow(row sqlcgen.SubmissionDraft) (SubmissionDraft, error) {
	...
	return SubmissionDraft{ID: row.ID, TenantID: row.TenantID, AssignmentID: row.AssignmentID, StudentID: row.StudentID, Content: content, UpdatedAt: timeFromPg(row.UpdatedAt)}, nil
}

// progressFromRow 转换学习进度行。
func progressFromRow(row sqlcgen.LessonProgress) LessonProgress {
	return LessonProgress{ID: row.ID, TenantID: row.TenantID, LessonID: row.LessonID, StudentID: row.StudentID, Status: row.Status, VideoPos: int32Value(row.VideoPos), DurationSec: row.DurationSec, UpdatedAt: timeFromPg(row.UpdatedAt)}
}

// postFromRow 转换讨论帖或回复行。
func postFromRow(row sqlcgen.DiscussionPost) DiscussionPost {
	return DiscussionPost{ID: row.ID, TenantID: row.TenantID, CourseID: row.CourseID, ParentID: int64Value(row.ParentID), AuthorID: row.AuthorID, Content: row.Content, IsPinned: row.IsPinned, LikeCount: row.LikeCount, CreatedAt: timeFromPg(row.CreatedAt)}
}

// announcementFromRow 转换公告表行。
func announcementFromRow(row sqlcgen.Announcement) Announcement {
	return Announcement{ID: row.ID, TenantID: row.TenantID, CourseID: row.CourseID, Title: row.Title, Content: row.Content, IsPinned: row.IsPinned, CreatedAt: timeFromPg(row.CreatedAt)}
}

// reviewFromRow 转换课程评价行。
func reviewFromRow(row sqlcgen.CourseReview) CourseReview {
	return CourseReview{ID: row.ID, TenantID: row.TenantID, CourseID: row.CourseID, StudentID: row.StudentID, Rating: row.Rating, Comment: textValue(row.Comment), CreatedAt: timeFromPg(row.CreatedAt)}
}

// weightFromCreateRow 转换创建权重返回行。
func weightFromCreateRow(row sqlcgen.CreateGradeWeightRow) GradeWeight {
	return GradeWeight{ID: row.ID, TenantID: row.TenantID, CourseID: row.CourseID, SourceType: row.SourceType, SourceRef: row.SourceRef, Weight: row.Weight, CreatedAt: timeFromPg(row.CreatedAt), UpdatedAt: timeFromPg(row.UpdatedAt)}
}
...
func gradeFromGetRow(row sqlcgen.GetCourseGradeRow) CourseGrade {
	return CourseGrade{ID: row.ID, TenantID: row.TenantID, CourseID: row.CourseID, StudentID: row.StudentID, AutoTotal: row.AutoTotal, OverrideTotal: row.OverrideTotal, IsOverridden: row.IsOverridden, IsLocked: row.IsLocked, UpdatedAt: timeFromPg(row.UpdatedAt)}
}
...
// int32Param 构造 int4 参数。
func int32Param(value int32) pgtype.Int4 {
	return pgtype.Int4{Int32: value, Valid: true}
}

// timestamptzParam 构造可空时间参数。
...
// timeFromPg 读取 pgtype 时间并统一转为 UTC。
