...
// NumericValue 读取必填 PostgreSQL numeric,数据库 NULL 或非法值显式返回错误。
func NumericValue(v pgtype.Numeric) (float64, error) {
	f, err := v.Float64Value()
	if err != nil {
		return 0, fmt.Errorf("读取 numeric 失败: %w", err)
	}
	if !f.Valid {
		return 0, fmt.Errorf("必填 numeric 为空")
	}
