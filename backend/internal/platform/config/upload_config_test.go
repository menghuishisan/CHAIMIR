...
func TestLoadReadsNotifyEventRetryConfig(t *testing.T) {
	...
	if cfg.Notify.EventRetryMax != 5 || cfg.Notify.EventRetryDelayMs != 250 || cfg.Notify.UnreadTTLHours != 72 {
...
// TestLoadRejectsMissingOperationalLimit 确认运行期阈值缺失时启动失败,不由代码默认值兜底。
