...
const finalitySource = ['function waitFinality(event) {', '  confirmations = head - event.height;', '  require(confirmations >= required);', '  require(!reorged(event.blockHash));', '  release(event.message);', '}'];
