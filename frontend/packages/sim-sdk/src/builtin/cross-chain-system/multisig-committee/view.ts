...
function committeeCells(state: CommitteeState): MatrixCell[][] {
  return matrixCells(state.members.map((member) => member.label), ['活跃', '签名', '有效'], (row, column) => {
    ...
    if (column === '活跃成员') return { label: member.active ? '是' : '否', status: member.active ? 'yes' : 'fault' };
    if (column === '签名份额') return { label: member.signed ? '已签' : '等待', status: member.signed ? 'yes' : 'pending' };
