 ...
 */
export function renderBlockValidationView(state: BlockValidationState): TeachingFrame {
  const failedItems = state.items.filter((item) => !item.valid);
    const summary = `区块 ${state.blockHash.slice(0, 8)},验证项 ${state.items.length},失败 ${failedItems.length},校验${state.accepted ? '通过' : '未通过'}。`;
  const patterns = [matrixPattern('block-validation-matrix', '区块头 / 交易根 / 收据根 / 状态根校验矩阵', state.items.map((item) => item.label), ['期望根', '实际根', '状态'], validationCells(state)), pipelinePattern('block-validation-pipeline', '本地重算各根并拒绝无效区块流程', pipelineSteps(blockValidationPhases, state.phaseIndex, state.items.some((item) => !item.valid)), blockValidationPhases[state.phaseIndex].id)];
  return teachingFrame({
    ...
    focus: {
      ...
      secondary: ['block-validation-matrix'],
    },
    layout: {
      primary: 'block-validation-pipeline',
      evidence: ['block-validation-matrix'],
    },
    patterns,
  });
}
