...
export function renderBlockValidationView(state: BlockValidationState): ViewSpec {
  return { summary: `区块 ${state.blockHash.slice(0, 8)},校验${state.accepted ? '通过' : '未通过'}。`, patterns: [matrixPattern('block-validation-matrix', '区块验证项', state.items.map((item) => item.label), ['期望', '实际', '状态'], validationCells(state), 'main'), pipelinePattern('block-validation-pipeline', '区块验证流程', pipelineSteps(blockValidationPhases, state.phaseIndex, state.items.some((item) => !item.valid)), blockValidationPhases[state.phaseIndex].id, 'bottom')] };
}
...
function validationCells(state: BlockValidationState): MatrixCell[][] {
  return matrixCells(state.items.map((item) => item.label), ['期望', '实际', '状态'], (row, column) => {
    const item = state.items.find((entry) => entry.label === row);
    if (!item) return { label: '无', status: 'empty' };
    if (column === '期望') return { label: item.expected.slice(0, 6), status: 'yes' };
    if (column === '实际') return { label: item.actual.slice(0, 6), status: item.valid ? 'yes' : 'fault' };
