...
const blockValidationSource = ['function validateBlock(block) {', '  require(block.parent == localTip.hash);', '  require(root(block.txs) == block.txRoot);', '  require(root(receipts) == block.receiptRoot);', '  require(execute(block).stateRoot == block.stateRoot);', '  acceptOrReject(block);', '}'];
