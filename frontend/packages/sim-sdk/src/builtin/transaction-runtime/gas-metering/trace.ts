...
const gasSource = ['function executeWithGas(tx) {', '  require(gasUsed <= gasLimit);', '  gasUsed += cost(op);', '  if gasUsed > gasLimit: revert();', '  refund = cappedRefund();', '  settleFee(gasUsed - refund);', '}'];
