 ...
 */
export function renderFlashLoanView(state: FlashLoanState): TeachingFrame {
  const priceShift = state.poolPrice - state.basePoolPrice;
  const debtGap = Math.max(0, state.protocolDebt - state.loanAmount);
    const summary = `借款 ${state.loanAmount},池价偏移 ${priceShift},协议债务缺口 ${debtGap},攻击利润 ${state.attackerProfit},限额${state.limitEnabled ? '已启用' : '未启用'}。`;
  const patterns = [graphPattern('flash-graph', '闪电贷借款 -> 操纵 -> 获利调用图', graphNodes(state.actors), graphEdges(state.calls)), lanePattern('flash-lane', '单笔原子交易内的组合调用时序', state.actors.map((actor) => actor.label), laneMessages(state.calls, (id) => labelOf(state, id)), state.tick), matrixPattern('flash-matrix', '闪电贷风控断点', ['单笔借款限额', '价格偏移保护', '冷却/延迟窗口'], ['结果'], flashCells(state))];
  return teachingFrame({
    ...
    focus: {
      primary: selectedOrFrameFocus(state.selectedElementId, ['flash-graph']),
      secondary: ['flash-lane', 'flash-matrix'],
    },
    layout: {
      primary: 'flash-graph',
      evidence: ['flash-matrix'],
      timeline: 'flash-lane',
    },
    patterns,
  });
