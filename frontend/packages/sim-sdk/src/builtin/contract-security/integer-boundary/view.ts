...
export function renderIntegerView(state: IntegerBoundaryState): ViewSpec {
  return { summary: `最大值 ${state.maxValue},溢出检查${state.checkedMath ? '已启用' : '未启用'},失败用例 ${state.cases.filter((item) => item.failed).length} 个。`, patterns: [matrixPattern('integer-matrix', '边界用例', state.cases.map((item) => item.label), ['输入', '结果', '溢出检查', '状态'], integerCells(state), 'main'), pipelinePattern('integer-pipeline', '整数安全流程', pipelineSteps(integerPhases, state.phaseIndex, state.cases.some((item) => item.failed)), integerPhases[state.phaseIndex].id, 'bottom')] };
}
...
function integerCells(state: IntegerBoundaryState): MatrixCell[][] {
  return matrixCells(state.cases.map((item) => item.label), ['输入', '结果', '溢出检查', '状态'], (row, column) => {
    ...
    if (column === '结果') return { label: item.input > state.maxValue && item.checked ? '已拒绝' : String(item.result), status: item.failed ? 'fault' : 'yes' };
    if (column === '溢出检查') return { label: state.checkedMath ? '是' : '否', status: state.checkedMath ? 'yes' : 'pending' };
