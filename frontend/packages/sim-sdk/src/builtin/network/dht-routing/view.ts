...
export function renderDhtView(state: DhtState): ViewSpec {
  ...
  return {
    summary: `查找 key ${state.lookupKey},跳数 ${state.hops},短名单 ${state.shortlist.length} 个,最近节点 ${closest?.label ?? '无'},最近距离 ${closest ? distance(closest, state.lookupKey) : '无'}。`,
    patterns: [
      graphPattern('dht-graph', 'Kademlia XOR 路由路径', graphNodes(state.peers), graphEdges(state.messages), 'main'),
      matrixPattern('dht-matrix', 'XOR 距离排序与候选短名单', state.peers.map((peer) => peer.label), ['节点 ID', 'K 桶', 'XOR 距离', '候选状态'], dhtCells(state), 'side'),
      pipelinePattern('dht-pipeline', '选择近邻 -> 查询 -> 收敛到最近节点流程', pipelineSteps([...dhtPhases], state.phaseIndex, state.shortlist.some((id) => state.peers.find((peer) => peer.id === id)?.polluted)), dhtPhases[state.phaseIndex].id, 'bottom'),
    ],
  };
}
