 ...
 */
export function renderDiscoveryView(state: DiscoveryState): TeachingFrame {
  const failedHandshake = state.peers.filter((peer) => peer.failedHandshakeReason).length;
    const summary = `已连接 ${state.peers.filter((peer) => peer.connected).length} 个节点,握手 ${state.handshakeCount} 次,失败握手 ${failedHandshake},黑名单 ${state.bannedPeerIds.length} 个。`;
  const patterns = [
      graphPattern('discovery-graph', 'P2P 地址发现与握手拓扑', graphNodes(state.peers), graphEdges(state.messages)),
      matrixPattern('discovery-matrix', '地址簿、握手健康与黑名单矩阵', state.peers.map((peer) => peer.label), ['地址数量', '握手', '健康', '黑名单'], discoveryCells(state)),
      pipelinePattern('discovery-pipeline', '发现地址 -> 握手 -> 健康过滤 -> 剔除流程', pipelineSteps([...discoveryPhases], state.phaseIndex, state.peers.some((peer) => (peer.malicious || peer.failedHandshakeReason) && !peer.banned)), discoveryPhases[state.phaseIndex].id),
    ];
  return teachingFrame({
    ...
    focus: {
      ...
      secondary: ['discovery-matrix', 'discovery-pipeline'],
    },
    layout: {
      primary: 'discovery-graph',
      evidence: ['discovery-matrix', 'discovery-pipeline'],
    },
    patterns,
  });
}
