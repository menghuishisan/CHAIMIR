 ...
 * 画布只负责形状,节点的信息完整性由下方节点清单承担;窄栏下画布压缩但清单不变。
 * 边(= 消息/调用)是无界元素:它的文本清单在右侧事件流里给全,不在舞台重复一份 ——
 * 那份清单随时间无上限增长,留在舞台会把图形顶出视口(§7.1 状态与事件分流)。
 */
...
import { EDGE_TONE } from "./elementStatus";
...
import type { GraphNode, GraphPattern } from "@chaimir/sim-sdk";
    ...
    <PatternFrame
      ...
      canvas={
        <svg
          ...
          role="presentation"
          className={cn("w-full", compact ? "max-h-40" : "min-h-0 flex-1")}
        >
          <defs>
