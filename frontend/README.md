...
- `apps/web`: 统一的单体前端 SPA 入口。采用 **FSD (Feature-Sliced Design)** 架构，其内部 `src/` 目录的严格职责划分如下：
  - `src/app/`: **驱动层**。负责路由树初始化、全局 Provider 挂载、鉴权守卫 (RoleGuard)、角色导航配置与全局样式引入。
  - `src/routes/`: **路由边界层 (极其重要)**。维护路由清单、懒加载页面入口和权限装配。这里可以引用 `features/*/pages` 页面实现，但不得编写业务逻辑、状态机或页面样式。
  - `src/layouts/`: **布局层**。只维护三大外壳：`AuthLayout` (登录/空旷页)、`MainLayout` (四角色共用的侧栏+顶栏标准工作台)、`ImmersiveLayout` (无侧栏的深色全屏工作台)。不得按角色复制布局。
  ...
  - `src/components/`: **应用级组件**。仅存放跨业务领域的 UI 拼装（如 `AppSidebar`, `NotificationBell`）。四角色导航只在 `src/app/roleNavigation.ts` 配置一次,侧栏、顶栏和登录落点共同读取该配置。注意：基础和通用业务组件应沉淀至 `packages/ui`。
  - `src/hooks/`: 应用级通用 Hooks（如 `useAuth`, `useWebSocket`）。
  - `src/store/`: 极轻量的全局状态（如 `currentUser`, `currentTenant`）。
  - `src/utils/`: 应用级工具（如本地存储键名定义、角色路径基础契约、日期格式化适配）。角色路径只在 `roleRouting.ts` 定义,`app/roleNavigation.ts` 仅为其装配菜单与图标。
