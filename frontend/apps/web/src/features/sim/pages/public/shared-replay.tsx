...
import type { JsonObject, RuntimeSnapshot, SimInitParams, SimPackageDescriptor } from '@chaimir/sim-sdk'
import type { SimActionLog, SimReplay } from '@chaimir/api-client'
...
/**
 * replayMoves 把记录动作摊平成逐步可控的命令序列。
 * 每条动作发生在特定推演时刻,而 Worker 只按命令推进时刻(见 sim.worker.ts applyEvent
 * 仅在 tick 事件后加一),所以注入之前必须先把时刻走到位 —— 顺序错了复现出的就不是原过程。
 * 摊平后一次命令对应 Worker 内一个事件,进度、单步与后退共用同一把尺子。
 */
function replayMoves(actions: SimActionLog[]): ReplayMove[] {
 ...
 */
function moveCommand(client: SimWorkerClient, move: ReplayMove): Promise<void> {
  if (move.kind === 'tick') {
    return client.step()
  }
