...
export function ContestFormModal({ contest, onClose, onSaved }: ContestFormModalProps) {
  ...
  const [signupStart, setSignupStart] = useState(toDateTimeInput(contest?.signup_start))
  const [signupEnd, setSignupEnd] = useState(toDateTimeInput(contest?.signup_end))
  const [startAt, setStartAt] = useState(toDateTimeInput(contest?.start_at))
  const [endAt, setEndAt] = useState(toDateTimeInput(contest?.end_at))
