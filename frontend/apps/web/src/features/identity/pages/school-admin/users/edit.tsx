    ...
    <div className={styles.page}>
      <div className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>账号资料和组织归属由后端校验并写入当前租户。</p>
        </div>
        {accountId && (
      ...
      <div className={styles.grid}>
        ...
        {!accountId && (
          <section className={styles.panel}>
            <h2>开通方式</h2>
            <Switch checked={useActivation} label={useActivation ? '使用激活码开通' : '使用初始密码开通'} onChange={(event) => setUseActivation(event.target.checked)} />
            {!useActivation && (
              <label className={styles.field}>
                初始密码
            ...
            <Callout variant="info" title="开通说明">
              使用激活码时，明文激活码只在创建结果中展示一次，请按学校流程安全下发。
            </Callout>
          </section>
        )}
