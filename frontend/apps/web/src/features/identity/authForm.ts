...
/** 手机号格式:与后端 identity ValidatePhone 的大陆手机号口径一致。 */
...
/** isPasswordStrong 与后端 ValidatePassword 保持同一强度口径,供所有身份表单复用。 */
...
/** isPhoneNumber 判断输入是否符合后端统一手机号规则。 */
...
export function phoneError(value: string, label: string): string | null {
  ...
  if (!isPhoneNumber(phone)) return `${label}格式不正确,请检查后重试`
  return null
}
