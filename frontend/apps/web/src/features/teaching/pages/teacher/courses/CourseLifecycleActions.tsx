...
import { Button, Callout, Input, Modal } from '@chaimir/ui'
...
export function CourseLifecycleActions({ course, onChanged }: { course: Course; onChanged: () => void }): React.ReactElement {
  ...
  const runAction = async (action: () => Promise<Course>, success: string) => {
    setError('')
    setMessage('')
    try {
    ...
    <>
      ...
      <Modal open={open} title={`${course.name} · 课程管理`} size="md" onClose={() => setOpen(false)}>
        ...
        <section className={styles.panel}>
          ...
          <Button variant="outline" icon={<KeyRound size={14} />} onClick={() => void runAction(() => api.teaching.refreshInviteCode(String(course.id)), '邀请码已刷新。')}>刷新邀请码</Button>
        </section>
        <section className={styles.panel}>
          <h2>复制课程</h2>
          <Input value={cloneName} onChange={(event) => setCloneName(event.target.value)} fullWidth />
          <Button variant="outline" icon={<Copy size={14} />} disabled={!cloneName.trim()} onClick={() => void runAction(() => api.teaching.cloneCourse(String(course.id), { name: cloneName.trim() }), '课程副本已创建。')}>创建副本</Button>
        </section>
        <div className={styles.actions}>
          <Button variant="outline" icon={<Share2 size={14} />} onClick={() => void runAction(() => api.teaching.shareCourse(String(course.id)), '课程已加入共享库。')}>共享课程</Button>
          <Button variant="outline" icon={<CheckCircle size={14} />} onClick={() => void runAction(() => api.teaching.endCourse(String(course.id)), '课程已结课。')}>结束课程</Button>
          <Button variant="danger" icon={<Archive size={14} />} onClick={() => { if (window.confirm('确定归档这门课程吗？')) void runAction(() => api.teaching.archiveCourse(String(course.id)), '课程已归档。') }}>归档课程</Button>
        </div>
      </Modal>
    </>
  )
