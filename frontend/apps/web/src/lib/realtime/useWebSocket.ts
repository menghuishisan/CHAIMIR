...
export function useWebSocket(
  ...
  const connect = useCallback(() => {
    if (!url) {
      reportHookError(new Error(REALTIME_ERROR_MESSAGES.INVALID_URL), 'connect')
      return
    }
  ...
  const send = useCallback(<T = unknown>(type: string, data: T) => {
    if (!clientRef.current) {
      reportHookError(new Error(REALTIME_ERROR_MESSAGES.NOT_CONNECTED), 'send')
      return
    }
  ...
  const on = useCallback(<T = unknown>(type: string, handler: WsEventHandler<T>) => {
    if (!clientRef.current) {
      reportHookError(new Error(REALTIME_ERROR_MESSAGES.NOT_CONNECTED), 'subscribe')
      return () => {}
    }
