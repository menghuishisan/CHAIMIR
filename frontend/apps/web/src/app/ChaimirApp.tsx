...
import { clearSession, getAccessToken, getStoredUser, getTraceId, readBrowserStorage, writeBrowserStorage } from '../lib/storage'
import { parseHashRoute, routeHref } from './router'
import { readFrontendConfig } from '../lib/config'
import { toUserFacingError } from '../lib/errors'
import type { UserFacingError } from '../lib/errors'
import { ERROR_ACTION_LABELS } from '../copy/errors'
...
class AppErrorBoundary extends React.Component<{ children: React.ReactNode }, { error?: UserFacingError }> {
  ...
  static getDerivedStateFromError(error: unknown): { error: UserFacingError } {
    return { error: toUserFacingError(error, { allowPlainMessage: false, titleKey: 'APP_CRASH' }) }
  }
    ...
    if (this.state.error) {
      return (
        <main className="chaimir-app-error" role="alert">
          ...
          <Button type="button" onClick={() => window.location.reload()}>{ERROR_ACTION_LABELS.RELOAD}</Button>
        </main>
      )
    }
...
function readSidebarCollapsed(app: AppDefinition): boolean {
  try {
    return readBrowserStorage(sidebarCollapsedStorageKey(app)) === 'true'
  } catch (error) {
...
function writeSidebarCollapsed(app: AppDefinition, collapsed: boolean): void {
  try {
    writeBrowserStorage(sidebarCollapsedStorageKey(app), String(collapsed))
  } catch (error) {
