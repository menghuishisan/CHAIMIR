...
const defaultContractAddress = process.env.CHAIMIR_EVENT_CONTRACT_ADDRESS || "";
const defaultEventTopic = process.env.CHAIMIR_EVENT_TOPIC || "";
...
app.get("/healthz", async () => ({ status: "ok", configured: Boolean(defaultContractAddress && defaultEventTopic) }));
app.get("/events/latest", { preHandler: verifyServiceSignature }, async (request, reply) => {
  const query = request.query || {};
  const contractAddress = String(query.address || defaultContractAddress).trim();
  const eventTopic = String(query.topic || defaultEventTopic).trim();
  if (!contractAddress || !eventTopic) {
    return reply.code(400).send({ error: "event address and topic are required" });
  }
  try {
    ethers.getAddress(contractAddress);
  } catch {
    return reply.code(400).send({ error: "event address is invalid" });
  }
